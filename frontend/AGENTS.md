...
This is the React SPA for the RAG Knowledge Base App. Read [../AGENTS.md](../AGENTS.md) first — universal building rules live there. This file adds frontend-specific conventions.
...
- **Auth:** JWTs issued by the FastAPI backend (email/password or magic links). No third-party auth providers (Google, Auth0) unless explicitly approved.
...
- **Validation:** only add a schema library (like Zod) when we actually need runtime validation at boundaries.
- **UI components:** shadcn primitives via `pnpm dlx shadcn@latest add <name>`. Don't hand-roll what shadcn already ships.
- **Data Visualization:** For the evaluation dashboard, prefer simple Tailwind/CSS bar charts. If a library is strictly necessary for complex RAGAS metric charts, use a lightweight, approved library like `recharts`. No heavy BI frameworks.
...
│   ├── lib/               # Framework-agnostic helpers (http, api, auth, env)
│   ├── pages/             # Route-level components (Chat, Dashboard, Upload, Settings)
│   ├── hooks/             # Custom React hooks (e.g., useChat, useDocumentUpload)
│   ├── App.tsx            # Router
│   ├── main.tsx
...
- **Streaming Chat:** The chat interface must support Server-Sent Events (SSE) or WebSockets for streaming LLM responses. Update the UI incrementally; do not wait for the full response to render.
- **Citations:** The LLM returns chunk IDs or reference markers. The frontend must map these to the UI to show source tooltips or highlight the exact text in the source document panel. Never render raw LLM output without parsing citations.
- **Document Upload:** Uploads must be asynchronous with clear progress indicators. Do not block the UI thread during file parsing or large uploads. Use standard `<input type="file">` or a lightweight drag-and-drop wrapper.
- **Evaluation Dashboard:** Display RAGAS metrics (faithfulness, relevancy, precision, recall) in clean, readable tables or simple charts. Focus on clarity over flashy animations.
...
- **Sanitize LLM Output:** If rendering markdown or HTML from the LLM, use a strict, approved sanitization library (like `DOMPurify`) to prevent XSS. Never use `dangerouslySetInnerHTML` with raw LLM output.
...
- Talks to a separate Python FastAPI backend over JSON. URL comes from `VITE_API_BASE_URL`.
- Always use `api.get/post/put/patch/delete` from `@/lib/api` — it handles base URL, JSON, Bearer token injection, timeouts, and typed `ApiError`s (including the `isNetworkError` flag that distinguishes CORS/network from HTTP errors).
- Auth is JWT-based via the FastAPI backend. The bearer token is injected automatically via the `api` client; never thread tokens through component props.
...
- Blocking the main thread during document upload or PDF parsing.
- Rendering raw, unsanitized LLM markdown/HTML directly into the DOM.
